...
const CONFIG_NAMESPACE = 'stylePastel';
...
export class HighlightManager implements vscode.Disposable {
    ...
    register(context: vscode.ExtensionContext): void {
        ...
        // Development 모드(F5로 띄운 Extension Host)에서만 declaration hover 노출 — 프로덕션에는 등록 안 함.
        if (context.extensionMode === vscode.ExtensionMode.Development) {
            const selectors: vscode.DocumentSelector = Array.from(SUPPORTED_LANGUAGES).map(
                language => ({ language })
            );
            context.subscriptions.push(
                vscode.languages.registerHoverProvider(selectors, {
                    provideHover: (document, position) => this.provideInspectHover(document, position),
                })
            );
        }
    ...
    private async toggleEnabled(): Promise<void> {
        ...
        vscode.window.setStatusBarMessage(
            `Style Pastel: ${!current ? 'enabled' : 'disabled'}`,
            2000
        );
    }

    /*
     * 디버그/검증용 hover. 등록 자체가 Development 모드에서만 일어나므로 프로덕션엔 노출되지 않음.
     * 위치한 declaration 1개를 찾아 property name → category label 과 raw declaration 을 보여준다.
     */
    private provideInspectHover(
        ...
        const label = hit.isCustomProperty
            ? 'CSS Variable'
            : (CATEGORY_STYLE_MAP.get(hit.category)?.label ?? hit.category);
        const propName = hit.raw.slice(hit.anchorStart - hit.start, hit.anchorEnd - hit.start);
        const rawOneLine = hit.raw.replace(/\s+/g, ' ').trim();

        const md = new vscode.MarkdownString();
        md.appendMarkdown(`**Style Pastel** — \`${propName}\` → ${label}\n\n`);
        md.appendCodeblock(rawOneLine, 'css');
