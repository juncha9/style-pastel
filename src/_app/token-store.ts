...
import { StyleToken } from '../_types';
import { extractStyleTokens } from '../_libs';
...
interface DocCache {
    ...
    readonly tokens: readonly StyleToken[];
}

// 변경이 같은 라인 안이고 추가/삭제 모두 줄넘김이 없는 경우만 incremental을 시도.
// 윈도우 = 변경 라인 ±LINE_BUFFER. 멀티라인 style 속성이 윈도우 밖으로 나가지 않는다는 가정.
const LINE_BUFFER = 5;
 ...
 *  ③ 그 외          → 전체 재추출
 *
 * Disposable이 아닌 이유: WeakMap이라 document가 GC되면 자동 정리됨.
 */
export class TokenStore {
     ...
     *  ④ 윈도우 substring을 extractStyleTokens로 재스캔하고 absolute offset으로 환산해 합친다.
     */
    private tryIncrementalRescan(
         ...
         *  - windowStart는 발견된 \n 위치에 둔다 — STYLE_ATTR_PATTERN의 leading `(?:\s|:|\()` 가
         *    substring 첫 매치에서도 정상 작동하도록 보장.
         */
        ...
        for (const t of cached.tokens) {
            ...
            if (t.start >= preEnd) {
                shifted.push({
                    ...
                    raw: t.raw,
                    category: t.category,
                    isCustomProperty: t.isCustomProperty,
                });
            } else {
                shifted.push(t);
            }
        ...
        // 윈도우 재스캔. lookbehind나 leading 문자 클래스 regex가 substring 시작점에서
        // 오작동하지 않도록 windowStart는 줄 시작 직전 \n 위치에 맞춰져 있다.
        const windowText = newText.slice(windowStart, windowEnd);
        const windowTokens = extractStyleTokens(windowText);
        const fresh: StyleToken[] = windowTokens.map(t => ({
